...
static char INPUT_AV_FILE[] = "input.avi"; // "input.jpg"; //"input.jpg"; //// Can take in either Video or Image file
#define MAX_OBJECTS_PER_FRAME (1000)
...
int main()
{
    ...
    while(true)
    {
        ...
        else
        {
            ...
            DPRINTF("==> Detected [%d] objects in [%f] seconds\n", numObjects, detectionTime.count());
            ...
            {    
                ...
                for (objId = 0; objId < numObjects; objId++)
                {
                    ...
                    if(x[0] == -1 && y[0] == -1)
                    {
                        x[0] = (leftTopX + rightBotX) / 2.0;
                        y[0] = (leftTopY + rightBotY) / 2.0;
                    }
                    else
                    {
                        x[0] = x[1];
                        y[0] = y[1];
                        x[1] = (leftTopX + rightBotX) / 2.0;
                        y[1] = (leftTopY + rightBotY) / 2.0;
                    }
                    auto end = now();
                    double ans = (x[1] - x[0]) * (x[1] - x[0]) + (y[0] - y[1]) * (y[0] - y[1]);
                    ans /= span(start, end);
                    ans /= 1000;
                    std::cout << ans << '\n';
                    ...
                    //printf("%s: %d %d %d %d\n", labels[objId].c_str(), leftTopX, leftTopY, rightBotX, rightBotY);
                    rectangle(image,
